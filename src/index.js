    ...
    visitor: {
      Program: function transform(path, state) {
        ...
        if (fileComments && fileComments.length) {
          if (
            fileComments.filter(
              comment => comment.value.indexOf("disable-encapsulation") !== -1
            ).length > 0
          ) {
      ...
      JSXOpeningElement: function transform(path, state) {
        ...
        const ignoredElements =
          state.opts.ignoredElements || defaultIgnoredElements;
        if (doesElementNameMatch(path.node.name, ignoredElements)) {
          return;
        }

        const className = makeClassNameFromPath(state.file.opts.filename);

        const classnameAttributes = path.node.attributes.filter(
          a =>
            t.isJSXAttribute(a) &&
        ...
        if (!classnameAttributes.length) {
          ...
          classnameAttributes.forEach(attr => {
            if (t.isStringLiteral(attr.value)) {
              ...
              throw new Error(
                "Babel Plugin Encapsulate JSX: unknown attribute type"
              );
            }
          });
        }
